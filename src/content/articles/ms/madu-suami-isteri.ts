import type { Article } from "../types";

export const article: Article = {
  slug: "madu-suami-isteri",
  title: "Madu Suami Isteri: Apa Sebenarnya Kandungannya? (Panduan Malaysia)",
  heading: "Apa itu madu suami isteri, dan apa yang patut ada di dalamnya?",
  description:
    "Panduan jujur tentang madu suami isteri di Malaysia: maksudnya, bahan yang biasa dicampurkan, cara membaca label sebelum membeli, dan cara pengambilannya.",
  primaryKeyword: "madu suami isteri",
  keywords: [
    "madu suami isteri",
    "madu untuk pasangan",
    "madu penyubur",
    "madu campuran herba",
    "madu suami isteri asli",
  ],
  published: "2026-07-23",
  updated: "2026-07-23",
  readingMinutes: 7,
  icon: "/icons/ingrediant_08.webp",
  figureAlt:
    "Sepasang suami isteri memegang sebalang madu SUBUR bersama.",
  category: "Panduan pembeli",
  blocks: [
    {
      type: "answer",
      question: "Apa itu madu suami isteri?",
      answer:
        "Madu suami isteri ialah madu yang dicampurkan dengan rempah panas, kekacang dan akar, bukan madu yang dimakan begitu sahaja. Di Malaysia ia dibeli oleh pasangan berkahwin sebagai amalan harian bersama, biasanya satu sudu kecil setiap pagi. Ia makanan harian, bukan ubat, dan resipinya berbeza-beza antara jenama.",
    },
    {
      type: "paragraph",
      text: "Masuk ke mana-mana farmasi, pasar atau senarai Shopee di Malaysia, anda akan jumpa balang berlabel madu suami isteri di sebelah madu kelulut dan madu tualang biasa. Harganya lebih mahal. Warnanya lebih gelap dan teksturnya lebih pekat. Tetapi hampir tiada apa-apa pada rak itu yang menerangkan apa sebenarnya yang membezakannya daripada madu di balang sebelah.",
    },
    {
      type: "paragraph",
      text: "Jawapan ringkasnya: madu suami isteri ialah madu campuran. Madu digunakan sebagai asas untuk membawa bahan-bahan lain. Bahan apa, dan berapa banyak, itulah seluruh perbezaan antara satu balang dengan balang yang lain — dan itulah satu-satunya perkara yang paling kabur pada kebanyakan label.",
    },
    {
      type: "heading",
      text: "Kenapa ia dipanggil madu \"suami isteri\"?",
      id: "kenapa-pasangan",
    },
    {
      type: "paragraph",
      text: "Nama itu menerangkan siapa yang membelinya, bukan fungsi perubatan. Di Malaysia dan rantau Nusantara yang lebih luas, makanan yang bersifat panas — halia, kayu manis, buah pala, habbatus sauda — diambil oleh kedua-dua pasangan bersama sebagai rutin kongsi, selalunya sebelum merancang keluarga atau sekadar sebagai amalan sepanjang perkahwinan. Gandingan itu bersifat budaya dan keagamaan, bukan sekadar pemakanan.",
    },
    {
      type: "callout",
      tone: "warning",
      title: "Perhatian tentang dakwaan yang anda lihat dalam talian",
      text: "Di bawah Peraturan-Peraturan Makanan 1985, produk makanan dan iklannya tidak dibenarkan mendakwa boleh mencegah, merawat atau menyembuhkan sebarang keadaan — termasuk kesuburan. Penjual yang menjanjikan kehamilan sedang melanggar undang-undang, bukan berkongsi rahsia. Anggap senarai seperti itu sebagai amaran tentang penjualnya, bukan tanda produk yang hebat.",
    },
    {
      type: "heading",
      text: "Apa yang sepatutnya ada dalam campuran madu suami isteri?",
      id: "bahan",
    },
    {
      type: "paragraph",
      text: "Tiada formula tetap, dan sebab itulah senarai bahan lebih penting daripada label di hadapan balang. Kebanyakan campuran dalam kategori ini mengambil daripada keluarga rempah panas, kekacang dan akar yang sama, tidak kira rantau ilhamnya. SUBUR menggunakan sepuluh:",
    },
    {
      type: "ingredients",
      title: "Sepuluh bahan dalam balang SUBUR",
      items: [
        { icon: "/icons/ingrediant_08.webp", name: "Sarang madu mentah", text: "Asasnya. Mentah, bukan diproses haba, jadi sarang masih ada dalam balang." },
        { icon: "/icons/ingrediant_02.webp", name: "Safron", text: "Bahan paling mahal dalam mana-mana campuran mengikut berat — dan yang pertama digugurkan oleh produk murah." },
        { icon: "/icons/ingrediant_10.webp", name: "Akar ginseng", text: "Digunakan untuk stamina dan tenaga harian." },
        { icon: "/icons/ingrediant_07.webp", name: "Habbatus sauda", text: "Bahan Sunnah yang paling dicari pembeli Malaysia mengikut nama." },
        { icon: "/icons/ingrediant_03.webp", name: "Halia segar", text: "Akar panas, biasanya diambil untuk peredaran darah." },
        { icon: "/icons/ingrediant_06.webp", name: "Kayu manis", text: "Rempah panas, dan sebab kenapa campuran yang baik berbau seperti itu apabila dibuka." },
        { icon: "/icons/ingrediant_01.webp", name: "Bunga lawang", text: "Biasanya diambil untuk penghadaman dan kepanasan badan." },
        { icon: "/icons/ingrediant_05.webp", name: "Buah pala", text: "Rempah panas klasik dalam masakan Nusantara." },
        { icon: "/icons/ingrediant_04.webp", name: "Walnut", text: "Padat nutrien, dan sebahagian sebab teksturnya pekat dan bukan cair." },
        { icon: "/icons/ingrediant_09.webp", name: "Badam", text: "Kekacang kedua, dikisar masuk ke dalam campuran, bukan sekadar ditabur di atas." },
      ],
    },
    {
      type: "paragraph",
      text: "Sepuluh itu di hujung tinggi. Banyak balang di pasaran hanya membawa tiga atau empat bahan dan bergantung pada madu untuk selebihnya. Kedua-duanya tidak salah — tetapi anda membayar untuk bahan tambahan itu, jadi berbaloi untuk tahu berapa banyak yang anda dapat.",
    },
    {
      type: "heading",
      text: "Bagaimana membaca label madu suami isteri?",
      id: "baca-label",
    },
    {
      type: "list",
      title: "Lima perkara yang patut disemak sebelum membayar",
      items: [
        { term: "Senarai bahan yang dinamakan", text: "Bukan \"herba pilihan\" atau \"resipi rahsia\". Kalau jenama enggan menamakan isinya, anda tidak boleh bandingkan dengan apa-apa." },
        { term: "Di mana safronnya", text: "Safron ialah bahan termahal dalam kategori ini. Kalau ia disenaraikan tetapi balangnya murah, ia hadir dalam jumlah yang amat sedikit." },
        { term: "Kenyataan halal yang jelas", text: "Dinyatakan dengan terang pada label atau oleh penjual. Bagi kebanyakan pembeli Malaysia, ini bukan pilihan." },
        { term: "Penjual di sebaliknya", text: "Perniagaan tempatan berdaftar yang boleh dihubungi lebih baik daripada senarai marketplace tanpa nama. SUBUR dijual menerusi Berkat Madinah Store." },
        { term: "Bahasa yang jujur", text: "Perkataan penggunaan lazim adalah sah dan biasa. Janji penyembuhan dan kehamilan tidak." },
      ],
    },
    {
      type: "table",
      caption: "Beza campuran penuh dengan campuran asas",
      columns: ["", "Campuran asas", "Campuran penuh"],
      rows: [
        ["Bahan", "3-4, selalunya tanpa nama", "8-10, setiap satu disenaraikan"],
        ["Safron", "Jarang disertakan", "Disertakan mengikut berat"],
        ["Asas madu", "Selalunya diproses haba", "Mentah, sarang masih ada"],
        ["Tekstur", "Cair, boleh dituang", "Pekat, perlu disudu"],
        ["Harga per 250g", "Lebih rendah", "Lebih tinggi — anda membeli bahan tambahannya"],
      ],
    },
    {
      type: "heading",
      text: "Bagaimana cara mengambilnya?",
      id: "cara-ambil",
    },
    {
      type: "steps",
      title: "Rutin pagi harian",
      steps: [
        { title: "Satu sudu kecil, perut kosong", text: "Sebelum apa-apa lagi pada waktu pagi. Sudu kecil, bukan sudu besar — ini amalan harian, bukan dos." },
        { title: "Tunggu kira-kira lima belas minit", text: "Kemudian bersarapan seperti biasa. Tempoh menunggu inilah yang paling kerap dilangkau." },
        { title: "Kedua-dua pasangan, waktu yang sama", text: "Rutin bersama itulah inti kategori ini. Mengambilnya bersama menjadikannya amalan, bukan sekadar suplemen." },
        { title: "Setiap hari, bukan sekali-sekala", text: "Penggunaan lazim mengandaikan konsistensi selama berminggu dan berbulan, bukan sebalang yang diambil sesekali." },
      ],
    },
    {
      type: "callout",
      tone: "note",
      title: "Penyimpanan",
      text: "Simpan balang di tempat sejuk dan kering, jauh daripada cahaya matahari terus, dengan penutup ditutup rapat. Campuran mentah yang mengandungi kekacang dan sarang akan berasing dan menjadi lebih gelap dari masa ke masa — itu normal, bukan rosak. Jangan simpan dalam peti sejuk; ia menjadikan campuran terlalu keras untuk disudu.",
    },
    {
      type: "heading",
      text: "Siapa yang tidak patut mengambilnya?",
      id: "siapa-elak",
    },
    {
      type: "list",
      items: [
        { term: "Kanak-kanak bawah satu tahun", text: "Madu mentah tidak pernah diberikan kepada bayi bawah dua belas bulan. Ini peraturan tegas, bukan sekadar langkah berjaga-jaga." },
        { term: "Sesiapa yang alah kekacang", text: "Walnut dan badam dikisar masuk ke dalam campuran dan tidak boleh diasingkan." },
        { term: "Mereka yang mengawal gula dalam darah", text: "Ia tetap madu. Berbincang dengan doktor sebelum menjadikannya amalan harian." },
        { term: "Sesiapa yang mengambil ubat preskripsi", text: "Ginseng dan habbatus sauda boleh berinteraksi dengan sesetengah ubat. Tanya doktor atau ahli farmasi dahulu." },
      ],
    },
    {
      type: "cta",
      heading: "Lihat apa yang sebenarnya ada dalam balang",
      text: "SUBUR menyenaraikan kesemua sepuluh bahan, asal usul campuran, dan cara pengambilannya — tiada resipi rahsia.",
      label: "Lihat senarai bahan",
    },
  ],
  faqs: [
    {
      q: "Apa itu madu suami isteri?",
      a: "Ia madu yang dicampurkan dengan rempah panas, kekacang dan akar — biasanya habbatus sauda, halia, kayu manis, buah pala, safron, ginseng dan kekacang kisar — dibeli oleh pasangan berkahwin di Malaysia sebagai amalan harian bersama, lazimnya satu sudu kecil setiap pagi. Ia makanan harian, bukan ubat.",
    },
    {
      q: "Adakah madu suami isteri membantu kesuburan?",
      a: "Tiada makanan yang boleh dijual di Malaysia dengan dakwaan mencegah, merawat atau menyembuhkan sebarang keadaan, termasuk kesuburan. Campuran ini diambil sebagai amalan harian. Jika anda ada kebimbangan tentang kehamilan, sila berjumpa doktor.",
    },
    {
      q: "Berapa banyak madu suami isteri patut diambil sehari?",
      a: "Satu sudu kecil pada waktu pagi ketika perut kosong, kira-kira lima belas minit sebelum sarapan. Ia diambil secara harian dan konsisten, bukan dalam kuantiti yang banyak.",
    },
    {
      q: "Boleh saya dan isteri ambil pada masa yang sama?",
      a: "Boleh, malah rutin bersama itulah inti kategori ini. Kedua-dua pasangan mengambil satu sudu kecil setiap pagi sebagai satu amalan, bukan dua suplemen berasingan.",
    },
    {
      q: "Bagaimana saya tahu madu suami isteri itu tulen?",
      a: "Cari senarai bahan yang dinamakan sepenuhnya dan bukan \"herba pilihan\", kenyataan halal yang jelas, penjual yang boleh dikenal pasti dan dihubungi, serta bahasa penggunaan lazim dan bukan janji penyembuhan. Campuran yang enggan memberitahu isinya tidak boleh dibandingkan dengan apa-apa.",
    },
    {
      q: "Adakah madu suami isteri halal?",
      a: "Ia bergantung kepada jenama, jadi semak kenyataan halal yang jelas pada balang atau terus daripada penjual. Madu dan rempah yang digunakan itu sendiri halal, tetapi cara produk dikendalikan dan dibungkus berbeza mengikut jenama.",
    },
    {
      q: "Berapa lama sebalang boleh bertahan?",
      a: "Pada kadar satu sudu kecil seorang sehari, balang 250g bertahan kira-kira tiga hingga empat minggu untuk sepasang. Disimpan sejuk, kering dan tertutup, balang yang belum dibuka tahan lama; pengasingan dan warna menjadi gelap adalah normal bagi campuran mentah.",
    },
  ],
  related: ["habbatus-sauda-dan-madu", "hadiah-madu-untuk-raya"],
};
